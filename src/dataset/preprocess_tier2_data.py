...
def filter_false_positive_polygons(
    pseudo_mask: np.ndarray, tier2_mask: np.ndarray, min_area: int = 50
) -> np.ndarray:
...
def process_annotation_pair(
    ...
    for i, crop in enumerate(crops):
        ...
        cv2.imwrite(
            str(crop_image_path), cv2.cvtColor(crop["image"], cv2.COLOR_RGB2BGR)
        )
        cv2.imwrite(str(crop_mask_path), (crop["mask"] * 255).astype(np.uint8))

        crop_paths.append(
            {"image": crop_image_path, "mask": crop_mask_path, "bbox": crop["bbox"]}
        )

    stats = {
        "original_pseudo_polygons": len(
            mask_to_polygons(pseudo_mask, min_area=min_area)
        ),
        "filtered_pseudo_polygons": len(
            mask_to_polygons(filtered_pseudo_mask, min_area=min_area)
        ),
...
def batch_process_tier2_data(
    ...
    if not image_files:
        raise ValueError(
            f"No images found in {images_dir} with extension {image_extension}"
        )
    ...
    for idx, image_path in enumerate(image_files, 1):
        try:
            ...
            if not predicted_mask_path.exists():
                if verbose:
                    print(
                        f"[{idx}/{len(image_files)}] Warning: Predicted mask not found for {image_path.name}"
                    )
                continue
            ...
            if not gt_mask_path.exists():
                if verbose:
                    print(
                        f"[{idx}/{len(image_files)}] Warning: GT mask not found for {image_path.name}"
                    )
                continue
            ...
            # Store results
            results.append({"image_name": image_path.name, "stats": stats})
            ...
            if verbose:
                print(
                    f"[{idx}/{len(image_files)}] Error processing {image_path.name}: {str(e)}"
                )
            results.append({"image_name": image_path.name, "error": str(e)})
    ...
    return {"summary": total_stats, "results": results}
