    ...
    def __init__(
        ...
        for i, out_ch in enumerate(intermediate_channels):
            ...
            self.conv_blocks.append(
                nn.Sequential(conv, nn.BatchNorm2d(out_ch), nn.ReLU(inplace=True))
            )
            in_ch = out_ch
    ...
    def forward(self, inputs: list[torch.Tensor]) -> torch.Tensor:
        ...
        for i, conv_block in enumerate(self.conv_blocks):
            ...
            if i > 0 and i - 1 < len(self.upsample_scales):
                x = F.interpolate(
                    x, scale_factor=self.upsample_scales[i - 1], mode="nearest"
                )

        # Apply final upsampling before classification
        if (
            len(self.conv_blocks) > 0
            and len(self.upsample_scales) > len(self.conv_blocks) - 1
        ):
            x = F.interpolate(x, scale_factor=self.upsample_scales[-1], mode="nearest")
