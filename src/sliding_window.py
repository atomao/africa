...
class SlidingWindowInference:
    ...
    def __call__(
        ...
        with torch.no_grad():
            dummy_window = image[:, :, :min(self.window_size[0], img_h), 
                                      :min(self.window_size[1], img_w)]
            dummy_out = model(dummy_window)
            num_classes = dummy_out.shape[1]
        
        # Initialize accumulation tensors
        logits_sum = torch.zeros(batch_size, num_classes, img_h, img_w, 
                                 device=device, dtype=torch.float32)
        weights_sum = torch.zeros(batch_size, 1, img_h, img_w, 
                                  device=device, dtype=torch.float32)
